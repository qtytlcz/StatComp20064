...
Apart from homework, we also wrote two functions **scadest** and **BinaryPoisson**. The first one realize the smoothly clipped absolute deviation penalty algorithm, And we can use the function to realize the simulation in the paper(Jianqing Fan and Runze Li,2001). The second one fit data with a mixed distribution of Poisson distribution using EM algorithm. This can fit a type of data more accurately, which recorded the number of occurrences of a random event in unit time.
